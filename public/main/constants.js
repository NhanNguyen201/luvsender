...
const C_NOISE_TEXT_VERTEXT = `
    ...
    void main(){
        float multi = pow(sin(uTime), 2.);
        vec3 distorsion = curlNoise(vec3(position.x + uTime * 1.2, position.y + uTime * 0.05, position.z + uTime * 0.05)) * multi;
        vec3 finalPos = ( position +  distorsion ) * (1. + multi);
        vec4 mvPosition = modelViewMatrix * vec4(finalPos, 1.);
        gl_PointSize = 3.;
        gl_Position = projectionMatrix * mvPosition;
    }
